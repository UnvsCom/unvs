...
#if defined(NDEBUG)
# error "UNVS cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xde34820303c228e483ad8385eca076991cc0c24629f2f0ca48644308cc14b1d1");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // UNVS: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "UNVS Signed Message:\n";
...
{
    // UNVS: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1)  
    {
        nSubsidy = 49349650 * COIN;
    }

    // Subsidy is cut in half every 525600 blocks, which will occur approximately every 5 years
    nSubsidy >>= (nHeight / 525600); // UNVS: 525600 blocks in 5 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // UNVS: 1 day
static const int64 nTargetSpacing = 5 * 60; // UNVS: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // UNVS: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // UNVS: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2735ad79c528c4ca194994cfc8c05b0cf45e30f28604784cdcbc0305a89d5320");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "UNVS relaunches a legend";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04cd6f6c19f525cbb903d092d5276f58b9deced951a365dd0b27f69ee4d6d1471740d4478763693f0e5fdef726370751070ea3e866e3251e4b5f2acc3bf0ee464c") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1677942179;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2095432392;

        if (fTestNet)
        {
            block.nTime    = 1677942165;
            block.nNonce   = 398895067;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x97a8c14cd534b822a28d106b37a1bc1f63c8e1cd3558996506ebfc224775829a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf8, 0xc2, 0xbu, 0xd1 }; // UNVS: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UNVSMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UNVSMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static UNVSMiner(CWallet *pwallet)
{
    printf("UNVSMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("unvs-miner");
    ...
    try { loop {
        ...
        printf("Running UNVSMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UNVSMiner, pwallet));
}
